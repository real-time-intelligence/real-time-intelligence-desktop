...
public class ServerHistorySCRP extends StackChartReportPanel {
    ...
    protected void loadData() {
        ...
        for (long dtBegin = chartRange.getBegin(); dtBegin <= chartRange.getEnd(); dtBegin += Math.round(range)) {
            ...
            try {
                sColumnListLocal = fStore.getSColumnListByCProfile(queryInfo.getName(), cProfile, dtBegin,
                        dtEnd);
            } catch (SqlColMetadataException | BeginEndWrongOrderException e) {
                throw new RuntimeException(e);
            }
            ...
            Map<String, IntSummaryStatistics> batchDataLocal = sColumnListLocal.stream()
                    ...
                    .collect(
                            Collectors.groupingBy(Map.Entry::getKey, Collectors.summarizingInt(Map.Entry::getValue)));
            ...
            series.forEach(series -> {
                ...
                try {
                    y = batchSize == 0 ? 0D : ((double) batch.map(IntSummaryStatistics::getSum)
                            .orElse(0L) / ((double) Math.round(range) / 1000));
